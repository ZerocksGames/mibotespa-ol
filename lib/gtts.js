...
function tokenize(text) {
  var text_parts = [];
  if (!text)
    throw new Error('No hay texto para decir');
  ...
  var server = http.createServer(function(req, res) {
    ...
    if (queryData && queryData.text) {
      ...
      res.end(JSON.stringify({
        code: -1,
        message: `Buscando texto. Por Favor Intenta. ${req.headers.host}?text=your+text`
      }))
    }
  });

  server.listen(port);
  console.log("Servidor de texto a voz que se ejecuta en " + port);
}
