...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] El código QR está listo, equipo subrek dulu yak ambipi`)
})
...
client.on('grupo-participantes-actualización', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `finalmente la carga del grupo se reduce 𝟭, adiós @${num.split('@')[0]} tus servicios serán enterrados profundamente`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ espera de nuevo el proceso',
				success: '️❬ ✔ ❭ listo 🖤',
				error: {
					stick: 'Bueno falló ;( , intenta repetir xd',
					Iv: 'lo siento enlace inválido☹️'
				},
				only: {
					group: '❬❗❭ SOLO GRUPO ',
					ownerG: '❬❗❭ SOLO OWNER ',
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'quotemaker':
					...
					client.sendMessage(from, buffer, image, {caption: 'Aquí anjim', quoted: mek})
					break
                 case 'phlogo':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
                ...
                case 'bug':
                     const pesan = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Lo siento texto demasiado largo, máximo 300 texto', msgType.text, {quoted: mek})
                    ...
                    break
                case 'ssweb':
					if (args.length < 1) return reply('¿Dónde está la url, amigo?')
					teks = body.slice(7)
					reply(mess.wait)
				...
				case 'cerpen':
					...
					break
				case 'chord':
					if (args.length < 1) return reply('¿Dónde está el texto, amigo?')
					tels = body.slice(7)
					anu = await fetchJson(`https://arugaz.herokuapp.com/api/chord?q=${tels}`, {method: 'get'})
					reply(anu.result)
					break
                case 'lirik':
                    if (args.length < 1) return reply('¿Dónde está el título de la canción, amigo?')
                    teha = body.slice(7)
                ...
                case 'anjing':
					...
					break
                case 'spamcall':
                   if (args.length < 1) return ('Ingrese el número de destino Bambang')
                   weha = body.slice(10)
				...
				case 'ocr': 
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('enviar fotos con ception ${prefix}ocr')
					}
					break
				case 'stiker': 
				case 'sticker':
				case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Envía fotos con subtítulos ${prefix}pegatina o etiqueta de respuesta / imagen`)
					}
					break
				case 'getses':
            ...
            break	
				...
				case 'tts':
					...
					if (args.length < 2) return client.sendMessage(from, '¿Qué texto quieres que suene? es mi voz?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('el texto significa bambangb 😤')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('Bueno falló;(, intenta repetir ^_^')
				...
				case 'setprefix':
					...
					break 
				case 'hilih': 
					if (args.length < 1) return reply('dame el texto xd!!!')
				...
				case 'tiktokstalk':
					try {
						...
						reply('[ERROR] posiblemente nombre de usuario no válido')
					}
					break
				case 'fitnah':	
				case 'fake':          
               if (!isGroup) return reply(mess.only.group)
				...
				case 'clearall':
					...
					reply('claro que todo es exitoso  :)')
					break
			       case 'block':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `Órdenes recibidas, bloq ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				    client.blockUser (`${body.slice(9)}@c.us`, "remove")
					client.sendMessage(from, `pedidos recibidos, abiertos ${body.slice(9)}@c.us`, text)
				break
				case 'leave': 
				if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				await client.leaveGroup(from, 'Byee', groupId)
                    break
				case 'bc': 
					if (!isOwner) return reply(' *LU ¿QUIÉN *?') 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ emisión pesada ❯\n\n${body.slice(4)}`})
						}
						reply('transmisión de éxito')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `❮ emisión pesada ❯\n\n${body.slice(4)}`)
						}
						reply('transmisión de éxito')
					}
					break
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('cambió con éxito el icono del grupo')
                break						
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`exitoso ?? Abre el Grupo Tod`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`tod grupo cerrado con éxito`)
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `si, tu puesto de administrador ha sido eliminado🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`si @${mentioned[0].split('@')[0]} su puesto de administrador ha sido eliminado`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Felicitaciones te convertiste en administrador de grupo :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Felicidades @${mentioned[0].split('@')[0]} te convertiste en administrador del grupo`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Asek recibe comida, otra patada :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Asek consiguió algo de comida, otra patada @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ activar la función simi en este grupo')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ desactivar la función simi en este grupo')
					} else {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ Activar la función welcome en este grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ Desactive la función welcome/left en este grupo')
					} else {
						reply(' *Ketik perintah 1 untuk mengaktifkan, 0 untuk menonaktifkan* \n *Contoh: ${prefix}welcome 1*')
					}
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' *LU QUIEN* ?') 
					if (args.length < 1) return reply(' *ETIQUETA QUE QUIERE CLONAR* ')
					...
					try {
						...
						reply(' *Bueno falló, intenta repetir xdd* ')
					}
					break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(' *ENVIAR FOTOS CON CAPTIO OCR* ')
					}
					break
				default:
